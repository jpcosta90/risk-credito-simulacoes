# Converted from Simulacao_v5.ipynb
# --- Cell 1 ---
import numpy as np
import random
import pandas as pd
...
def atualiza_mes(anterior, ctr_dia=50):
    ...
    for i in range(len(anterior)):
        ...
        if anterior[i].atraso == 0:
            ...
            p = 0.55 + 0.10 * (temp_atraso - 30)/30
        elif anterior[i].atraso < 90:    
            p = 0.70 + 0.05 * (temp_atraso - 60)/30
        elif anterior[i].atraso < 120:
            p = 0.76 * anterior[i].mtpl_rec
        elif anterior[i].atraso < 150:
            p = 0.78 * anterior[i].mtpl_rec
