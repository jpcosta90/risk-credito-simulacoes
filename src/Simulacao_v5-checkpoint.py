# Converted from Simulacao_v5-checkpoint.ipynb
# --- Cell 1 ---
import numpy as np
import random
import pandas as pd
...
class cliente:
 ...
 #       self.inad = max(0.02 + np.random.normal(0., 0.005),0.003)
        ...
        self.inad = max(0.012 + np.random.normal(0., 0.005),0.0003) * chave_prob[self.porte]
...
def atualiza_mes(anterior, ctr_dia=50):
    ...
    for i in range(len(anterior)):
        ...
        if anterior[i].atraso == 0:
            ...
            p = 0.10 + 0.40 * (temp_atraso - 0)/30
        elif anterior[i].atraso < 60:
            p = 0.50 + 0.10 * (temp_atraso - 30)/30
        elif anterior[i].atraso < 90:    
            p = 0.60 + 0.05 * (temp_atraso - 60)/30
        elif anterior[i].atraso < 120:
            p = 0.70 * anterior[i].mtpl_rec
        elif anterior[i].atraso < 150:
            p = 0.75 * anterior[i].mtpl_rec
        elif anterior[i].atraso < 180:
            p = 0.80  * anterior[i].mtpl_rec
        ...
        if atual[i].modalidade == 4 and anterior[i].atraso > 90 and p_atraso == 0:
                    ...
                    atual[i].valor = 0
                    atual[i].prazo = 0
        elif anterior[i].atraso > 90 and p_atraso == 0:
            atual[i].p_inad = p*(0.5)      
...
sql = "drop view jpcosta.calculo_lgd ;"
cur.execute(sql)
